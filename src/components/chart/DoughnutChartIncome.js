        ...
        return (
            <>  
                <image
                    ...
                    width={28}
                    height={28}
                    xlinkHref={data?.listIncome[index].icon}
                />
                <text x={x1-1} y={y1-6} dy={10} textAnchor="middle" fill={colors[index]} className="text-xs font-semibold">{((data?.listIncome[index].totalAmount / data?.totalIncome)* 100).toFixed(1)}%</text>
            </>
        );
    };
    ...
    return (
        <div style={{ pointerEvents: 'none' }}>
            <PieChart width={300} height={300} margin={{ top: 30, right: 30, bottom: 30, left: 30 }}>
                ...
                >
                    {data?.listIncome && data.listIncome.map((entry, index) => (
