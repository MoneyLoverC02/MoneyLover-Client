    ...
    <React.Fragment>
      <button onClick={handleOpen}>
        <div className='flex justify-center items-center mr-4 py-[11px] pl-4 pr-3 border-[1px] border-gray-300 rounded-lg hover:border-gray-500'>
          <img src={iconSelect ? iconSelect.icon : 'https://static.moneylover.me/img/icon/icon.png'} className='object-cover w-10 h-10 mr-[10px]' alt="" />
      ...
      >
        <Box sx={{ ...style, width: 496, minHeight: 500 }}>
          ...
          <div className='flex justify-center items-center border-b-[0.5px] border-slate-300'>
            <span className='uppercase text-sm px-6 py-2 text-lightgreen font-medium cursor-pointer'>Basic</span>
          </div>
          <div className='grid grid-cols-8 scroll-smooth'>
            {iconWallets && iconWallets.map(icon => (
              <span key={icon.id} className='flex justify-center p-2 cursor-pointer hover:bg-lime-50' onClick={() => handleChoosenIcon(icon.id)}>
