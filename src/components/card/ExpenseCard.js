...
export default function ExpenseCard({ balance, isOpen, onClose }) {
    ...
    const [dataForChart, setDataForChart] = useState ({listExpense: [], totalExpense: 0});

    useEffect(() => {
        let totalExpense = 0;
        let listExpense = dataCalculated?.listExpense;
        listExpense.forEach(item => {
            totalExpense -= item.totalAmount
        })
        setDataForChart({listExpense, totalExpense})
    }, [dataCalculated])
                    ...
                    <div className="border-y grid grid-cols-2 pb-8 pl-8">
                        <div className="w-full">
                            <div className='w-full flex justify-center'>
                                <DoughnutChartExpense data={dataForChart} />
                            </div>
                        </div>
                        <div className="text-3xl flex justify-end items-center mr-20 text-red-500">
                            <span>
                                {numeral(balance?.totalExpense).format('0,0')} {walletSelect?.currency.sign}
                            </span>
                        </div>
                    </div>
                    <div className="pb-6 text-sm font-medium mt-2 text-textgray">
                        {dataCalculated?.listExpense.length > 0 && dataCalculated.listExpense.map(item => {
                            return (
