            ...
            <button onClick={handleOpen}>
                ...
                <div className='wrap-text-icon'>
                    <div onClick={handleSelectCategory} className='flex justify-center items-center'>
                        {categorySelect ?
                            (<>
                                <img src={categorySelect.icon} className="w-6 h-6 object-cover mr-4 rounded-full" alt='icon-flag' />
                                <span className="text-input text-start">{categorySelect.name}</span>
                            </>
                            ) :
                            (<>
                                <img src='https://static.moneylover.me/img/icon/icon_not_selected.png' className="w-6 h-6 object-cover mr-4 rounded-full" alt='icon-flag' />
            ...
            >
                <Box sx={{ ...style, width: 550, minHeight: 500 }}>
                    ...
                    <div className=" h-[48px] fomt-normal border-b flex justify-center mx">
                        <button className="w-full py-[15px] uppercase leading-4 text-sm font-semibold text-zinc-400">Debt/Loan</button>
                        {!selectInCome ?
                            <>
                                <button onClick={handleSelectExpense} className="w-full py-[15px] uppercase leading-4 text-sm font-semibold border-b border-lightgreen text-lightgreen">Expense</button>
                                <button onClick={handleSelectInCome} className="w-full py-[15px] uppercase leading-4 text-sm font-semibold text-zinc-400">Income</button>
                            </>
                            :
                            <>
                                <button onClick={handleSelectExpense} className="w-full py-[15px] uppercase leading-4 text-sm font-semibold text-zinc-400">Expense</button>
                                <button onClick={handleSelectInCome} className="w-full py-[15px] uppercase leading-4 text-sm font-semibold border-b border-lightgreen text-lightgreen">Income</button>
                            </>
                        }
                    </div>
